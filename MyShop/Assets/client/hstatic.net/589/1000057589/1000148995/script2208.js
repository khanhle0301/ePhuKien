	...
	if(jsonProIndex != null & jsonProducts != null & $current_product != null )
	{
		...
		for (i=0; i<arrProIndex.length; i++ )
		{
			...
			{ 
				...
				+'</div>'
				+'<ul class="entry-meta"><li class="color">'+ '<ins> '+price+' </ins>'+ old_price +'</li></ul>'
...
Haravan.onSuccess = function(fly_img, form_id) { 
	...
	if($boUpdated == false){ // current item is not existed!!!  
		...
		+'<a class="top_cart_item_remove" onclick = "deleteCart('+ $product_page.find('[name="id"]').val() +');"><i class="icon-remove-sign"></i></a>'
		+ ' </div>'
		+ '</div>';
	...
	$('.cart_total').html(Haravan.formatMoney($price_new, "") + 'đ');
};
	...
	jQuery.getJSON('/cart.js', function(cart, textStatus) {
		if(cart)
		{
			...
			if(cart.item_count > 0){
				...
				/*
					$('.cart_total').html(Haravan.html(Haravan.formatMoney(cart.total_price, "") + 'đ'));
			*/
			}
			else {
				$('.top-cart-block #top-cart-trigger span').html(cart.item_count);
				...
				//Vinh edited 20160803 #T2V
				$('.cart_total').html(Haravan.formatMoney(cart.total_price, "") + 'đ');
		}
		else {
			$('.top-cart-block .top-cart-content').html(top_cart_empty);
			$('.top-cart-block .top-cart-content').html(top_cart_empty); 
